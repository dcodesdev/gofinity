...
func Wait(ctx context.Context, ready <-chan struct{}) error {
	...
	case <-ready:
		return nil
	case <-ctx.Done():
		// ctx.Err() is never nil once Done is closed, so this is the one place
...
func Produce(ctx context.Context, values []int) <-chan int {
	out := make(chan int)
	go func() {
		// The producer owns the channel, so the producer closes it - and the
		// defer means it is closed on the cancelled path too.
		defer close(out)
