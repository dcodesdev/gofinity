...
func Produce(n int) <-chan int {
	out := make(chan int)
	go func() {
		// defer, so the channel is closed even if a send panics, and so the
		// close is impossible to forget on an early return.
		defer close(out)
		for i := range n {
			out <- i
		}
	}()
	// Returning out as <-chan int hands the caller the receiving end only: it
	// cannot send and it cannot close, which is exactly the contract.
	return out
}
