...
func DecodeTask(data []byte) (Task, error) {
	var task Task
	// The second argument must be a pointer: Unmarshal writes through it, and
	// passing the value itself is an InvalidUnmarshalError rather than a
	// silent no-op.
...
func LookupInt(m map[string]any, key string) (int, bool) {
	// Two comma-ok forms back to back: one for the map, one for the assertion.
	v, ok := m[key]
	if !ok {
		return 0, false
	}
...
func DecodeStream(r io.Reader) ([]Task, error) {
	...
	for {
		...
		if errors.Is(err, io.EOF) {
			// The stream ended cleanly between values. Anything else is a
			// real failure and has to come back to the caller.
			return tasks, nil
		}
		if err != nil {
			return nil, err
