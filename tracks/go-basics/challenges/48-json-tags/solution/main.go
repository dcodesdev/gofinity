...
func (c Celsius) MarshalJSON() ([]byte, error) {
	...
	// same float64: 21.5 stays 21.5, and 0 is "0" rather than "0.0".
	text := strconv.FormatFloat(float64(c), 'f', -1, 64) + "C"
	// Marshalling the string rather than writing the quotes by hand means the
	// escaping is somebody else's problem.
...
func (c *Celsius) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &text); err != nil {
		return fmt.Errorf("celsius: not a JSON string: %w", err)
	}

	number, ok := strings.CutSuffix(text, "C")
	if !ok {
		return fmt.Errorf("celsius: %q is missing the C suffix", text)
	}

	value, err := strconv.ParseFloat(number, 64)
	if err != nil {
		return fmt.Errorf("celsius: %q: %w", text, err)
	}
