...
func parseScore(raw string) (int, error) {
	if raw == "" {
		return 0, fmt.Errorf("parse %q: %w", raw, ErrInvalid)
	}
	n := 0
	for _, r := range raw {
		if r < '0' || r > '9' {
			// %w keeps ErrInvalid reachable; %v would flatten it to text.
			return 0, fmt.Errorf("parse %q: %w", raw, ErrInvalid)
		}
		n = n*10 + int(r-'0')
	}
...
func Score(name string) (int, error) {
	raw, err := fetch(name)
	if err != nil {
		return 0, fmt.Errorf("score %q: %w", name, err)
	}
	n, err := parseScore(raw)
	if err != nil {
		return 0, fmt.Errorf("score %q: %w", name, err)
	}
	return n, nil
...
func Unwrapped(err error) error {
	for {
		...
		if inner == nil {
			// Either err is nil, or it is the bottom of the chain.
			return err
		}
		err = inner
	}
