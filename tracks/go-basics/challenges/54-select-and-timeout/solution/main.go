...
func TryRecv(ch <-chan int) (int, bool) {
	...
	case v := <-ch:
		return v, true
	default:
		// No case was ready, and with a default select does not wait for one.
		return 0, false
	}
}
...
func Merge(chs ...<-chan int) <-chan int {
	...
	go func() {
		// Wait blocks until the last input goroutine has returned, which is
		// the only moment at which closing out is safe: a send on a closed
		// channel panics.
		wg.Wait()
		close(out)
	}()
...
func GenerateUntil(done <-chan struct{}) <-chan int {
	...
	go func() {
		...
		for i := 0; ; i++ {
			...
			case out <- i:
			case <-done:
				// Closing done makes this case ready for every goroutine
				// watching it at once, which is why cancellation is a close
				// and never a send.
				return
			}
		}
